...
tricorr_ROC, loaded_tricorr_params = load_or_calculate_multipatient_ROC(patients_considered, "tricorr", tricorr_signals_reduction_name; tricorr_params...)
aEEG_ROC, loaded_aEEG_params = load_or_calculate_multipatient_ROC(patients_considered, "aEEG", aEEG_signals_reduction_name; aEEG_params...)
@warn "FIXME New plotting functions!!!"
fig = Figure(resolution=resolution)

tricorr_color = :blue
aEEG_color = :red
combined_plt = plot_NODRAW_seizure_detection_ROC!(tricorr_ROC, color=tricorr_color, epoch_s=aEEG_params[:epoch_s]) + plot_NODRAW_seizure_detection_ROC!(aEEG_ROC, color=aEEG_color, epoch_s=tricorr_params[:epoch_s])
