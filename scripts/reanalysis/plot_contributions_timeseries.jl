...
snippets_duration = 1
save_prefix = "tricorr_ts_zscore_zscore_IndStdNormal_None_snippets$(snippets_duration)_lagextents8x25_helsinkiEEG"
plots_subdir = plotsdir("$(save_prefix)_$(Dates.format(Dates.now(), "yyyy_mm_dd-HHMMSS"))")
mkpath(plots_subdir)

target_match_patient = patient -> "tricorr_ts_zscore_zscore_IndStdNormal_None_snippets$(snippets_duration)_lagextents8x25_helsinkiEEG$(patient)_"
...
for (i, contributions) ∈ enumerate(contributions_by_patient)
    ...
    times = get_times(eeg, sample_rate=1/snippets_duration)
